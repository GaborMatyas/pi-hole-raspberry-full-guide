...
#### If the pihole does not download and refresh the adlist with the previous steps
Create a resolv.conf file in the folder where the docker-compose.yml file exists on raspberry and paste this in it. 
```
# Generated by NetworkManager
search home
nameserver 127.0.0.1
```
Add a volume in docker-compose.yml file pointing this file /etc/resolv.conf
Should look like:
```
 volumes:
      - './etc-pihole:/etc/pihole'
      - './etc-dnsmasq.d:/etc/dnsmasq.d'
      - './resolv.conf:/etc/resolv.conf'  # this is the pasted line 
```
